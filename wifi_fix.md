...
To install CachyOS you need an Ethernet cable or a WiFi dongle, as the integrated WiFi based on the chipset **BCM43602** is available but connecting to a network is not possible because the password is constantly requested.

### Step 1: Disable problematic feature

Create or edit `/etc/modprobe.d/brcmfmac.conf`:

```bash
echo 'options brcmfmac feature_disable=0x82000' | sudo tee /etc/modprobe.d/brcmfmac.conf
```

### Step 2: Add firmware configuration

Create the file `/usr/lib/firmware/brcm/brcmfmac43602-pcie.txt` with the following content.

> **Important:** Replace `xx:xx:xx:xx:xx:xx` with your actual MAC address, and `ccode=NL` with your country code (e.g., `US`, `GB`, `DE`, etc.).
...
After reboot, WiFi should connect normally without constantly asking for the password.

---

## Conclusion

So far, **CachyOS has been the best distro I have tried on this hardware.**
